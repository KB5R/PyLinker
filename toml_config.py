...
def dell_hosts():
    ...
    selected = radiolist_dialog(
        title="Выбор хоста",
        text="Выберите хост для удаления:",
        values=host_choices
    ).run()
    ...
    details = config[group][host_name]
    user = details.get("user", "???")


    confirm = yes_no_dialog(
        title="Подтверждение",
        text=f"Удалить хост '{host_name}' из группы '{group}'?"
    ).run()

    if not confirm:
        message_dialog(title="Отмена", text="Удаление отменено.").run()
        return


    if details.get("password_storage") == "keyring":
        service_name = f"pylinker_{group}_{host_name}"
        try:
            keyring.delete_password(service_name, user)
        except keyring.errors.PasswordDeleteError:
            pass  # пароль уже был удалён или не найден
